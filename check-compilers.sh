...
output="supported_cxx_compilers.txt"
if [[ -L "$output" ]]; then
...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/p101-check-cxx.XXXXXX")"
trap 'rm -rf -- "$tmp_dir"' EXIT
printf '%s\n' 'int main() { return 0; }' > "$tmp_dir/probe.cpp"
...
for path_dir in $PATH; do
  ...
  shopt -s nullglob
  for candidate in \
    "$path_dir"/g++ "$path_dir"/g++-[0-9]* "$path_dir"/g++[0-9]* \
...
for candidate in "${candidates[@]:-}"; do
  if "$candidate" "$tmp_dir/probe.cpp" -o "$tmp_dir/probe" >/dev/null 2>&1; then
    usable+=("$candidate")
  fi
done
